...
export class initDB1647762523830 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`ideas\` (\`id\` int NOT NULL AUTO_INCREMENT, \`creator_id\` int NOT NULL, \`topic_id\` int NOT NULL, \`title\` text NULL, \`description\` text NULL, \`is_incognito\` tinyint(1) NOT NULL DEFAULT '0', \`created_at\` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`delete_flag\` tinyint(1) NOT NULL DEFAULT '0', UNIQUE INDEX \`IDX_6ab43f1e9b1cef0d8f3e56ce3a\` (\`id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`topics\` (\`id\` int NOT NULL AUTO_INCREMENT, \`creator_id\` int NOT NULL, \`name\` text NULL, \`description\` text NULL, \`lock_date\` timestamp NULL, \`close_date\` timestamp NULL, \`created_at\` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`delete_flag\` tinyint(1) NOT NULL DEFAULT '0', UNIQUE INDEX \`IDX_e4aa99a3fa60ec3a37d1fc4e85\` (\`id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`ideas\` ADD CONSTRAINT \`FK_736a14f3fdf2c6c6315e6b47e7f\` FOREIGN KEY (\`topic_id\`) REFERENCES \`topics\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        ...
        await queryRunner.query(`ALTER TABLE \`topics\` ADD CONSTRAINT \`FK_4f2581d11c46de072a66b2f1a97\` FOREIGN KEY (\`creator_id\`) REFERENCES \`users\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
